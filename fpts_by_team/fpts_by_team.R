...
ui <- fluidPage(
  h1('First Player To Score By Team'),
  br(),
  fluidRow(
    column(
      width = 3,
  ...
  fluidRow(
    column(
      width = 10,
      h4(textOutput('fpts_by_team_last_update')))),
  br(),
  fluidRow(reactableOutput('fpts_by_team_table'))
)
...
server <- function(input, output, session) {
  ...
  fpts_by_team_last_update <- eventReactive(input$updateButton, {
    max(as_datetime(fpts_by_team_raw()$run_timestamp_utc))
  })
  # extract and render datetime
  output$fpts_by_team_last_update <- renderText(
